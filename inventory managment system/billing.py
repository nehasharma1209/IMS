...
class BillClass:
    def __init__(self,root):
        ...
        self.icon_title=self.icon_title.resize((64,64),resample=Image.LANCZOS)
        ...
        #===btn_logout===
        btn_logout=Button(self.root,command=self.logout,text="Logout",font=("times new roman",15,"bold"),bg="#7AC5CD",cursor="hand2").place(x=1110,y=10,height=50,width=150)
        ...
        btn_search=Button(ProductFrame2,text="Search",command=self.search,font=("goudy old style",15),bg="#2196f3",fg="white",cursor="hand2").place(x=286,y=45,width=100,height=25)
        btn_show_all=Button(ProductFrame2,text="Show All",command=self.show,font=("goudy old style",15),bg="#083531",fg="white",cursor="hand2").place(x=286,y=10,width=100,height=25)
        ...
        lbl_note=Label(ProductFrame1,text="Note:'Enter 0 Quantity to remove product frome the Cart'",font=("gouddy old style",12),anchor='w',bg="white",fg="red").pack(side=BOTTOM,fill=X)
        ...
        Add_cartwidgets=Frame(self.root,bd=2,relief=RIDGE ,bg="white")
        Add_cartwidgets.place(x=420,y=550,width=530,height=110)

        lbl_p_name=Label(Add_cartwidgets,text="Product Name",font=("time new roman",15),bg="white").place(x=5,y=5)
        txt_p_name=Entry(Add_cartwidgets,textvariable=self.var_pname,font=("time new roman",15),bg="lightyellow",state='readonly').place(x=5,y=35,width=190,height=22)

        lbl_p_price=Label(Add_cartwidgets,text="Price Per Qty",font=("time new roman",15),bg="white").place(x=230,y=5)
        txt_p_price=Entry(Add_cartwidgets,textvariable=self.var_price,font=("time new roman",15),bg="lightyellow",state='readonly').place(x=230,y=35,width=150,height=22)

        lbl_p_qty=Label(Add_cartwidgets,text="Quantity",font=("time new roman",15),bg="white").place(x=390,y=5)
        txt_p_qty=Entry(Add_cartwidgets,textvariable=self.var_qty,font=("time new roman",15),bg="lightyellow").place(x=390,y=35,width=120,height=22)

        self.lbl_inStock=Label(Add_cartwidgets,text="In Stock",font=("time new roman",15),bg="white")
        self.lbl_inStock.place(x=5,y=70)

        btn_clear_cart=Button(Add_cartwidgets,text="Clear",command=self.clear_cart,font=("times new roman",15,"bold"),bg="lightgray",cursor="hand2").place(x=180,y=70,width=150,height=30)
        btn_add_cart=Button(Add_cartwidgets,text="Add | Update Cart",command=self.add_update_cart,font=("times new roman",15,"bold"),bg="orange",cursor="hand2").place(x=340,y=70,width=180,height=30)
        ...
        btn_generate=Button(billMenuFrame,text='Genrate Bill/Save Bill',command=self.generate_bill,cursor="hand2",font=('goudy old style',12,'bold'),bg="#009688",fg="white")
        btn_generate.place(x=246,y=80,width=160,height=50)

        #===============footer===================
 
        footer=Label(self.root,text="IMS-Inventory Management System | Developed By Team-7\nFor any Technical Issue contact:834xxxxx0",font=('times new roman',11),bg="#4d636d",fg="white").pack(side=BOTTOM,fill=X)
...
#================All Function===============================
            ...
            if present == 'yes':
                ...
                if op==True:
                    if self.var_qty.get()=="0":
                        ...
                        self.cart_list[index_][3]=self.var_qty.get() #Quality 
            else:
    ...
    def bill_updates(self):
        ...
        # self.net_pay=self.bill_amnt-self.lbl_discount
        ...
        try:
            ...
            # pid,name,price,qty,status
               ...
               # ================Update Qty in Product Table================
                ...
                con.commit()
                # self.txt_bill_area.insert(END,self.bill_middle)
            self.show()
        except Exception as ex:
            messagebox.showerror("Error",f"Error due to : {str(ex)}",parent=self.root)
