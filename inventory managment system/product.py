...
class productClass:
    def __init__(self,root):
        ...
        lbl_product=Label(product_Frame,text="Name",font=("goudy old style",18)).place(x=30,y=160)
        ...
        cmb_status=ttk.Combobox(product_Frame,textvariable=self.var_status,values=("Active","Inative"),state='readonly',justify=CENTER,font=("goudy old style",15))
        cmb_status.place(x=150,y=310,width=200)
        cmb_status.current(0)

        #====button 
        btn_add=Button(product_Frame,text="Save",command=self.add,font=("goudy old style",15,"bold"),cursor="hand2",bg="#2196f3",fg="white").place(x=10,y=400,width=100,height=40)
        btn_update=Button(product_Frame,text="Update",command=self.update,font=("goudy old style",15,"bold"),cursor="hand2",bg="green",fg="white").place(x=120,y=400,width=100,height=40)
        btn_delete=Button(product_Frame,text="Delete",command=self.delete,font=("goudy old style",15,"bold"),cursor="hand2",bg="red",fg="white").place(x=230,y=400,width=100,height=40)
        btn_clear=Button(product_Frame,text="Clear",command=self.clear,font=("goudy old style",15,"bold"),cursor="hand2",bg="grey",fg="white").place(x=340,y=400,width=100,height=40)

        #======searchFrame==========

        SearchFrame=LabelFrame(self.root,text="Search ",font=("goudy old style",15,"bold"),bd=2,relief=RIDGE,bg="honeydew1")
        SearchFrame.place(x=480,y=10,width=600,height=80)

        #=========option=========
        cmb_search=ttk.Combobox(SearchFrame,textvariable=self.var_emp_searchby,values=("select","Category","Supplier","Name"),state='readonly',justify=CENTER,font=("goudy old style",15))
        cmb_search.place(x=10,y=10,width=180)
        cmb_search.current(0)


        txt_search=Entry(SearchFrame,textvariable=self.var_emp_searchtxt,font=("goudy old style",15,"bold"),bg="lightyellow").place(x=200,y=10)
    ...
    def search(self):    
        ...
        try:
            if self.var_emp_searchby.get()=="select":
                ...
                cur.execute("select * from product where "+self.var_emp_searchby.get()+" LIKE '%"+self.var_emp_searchtxt.get()+"%'")
                rows=cur.fetchall()
                if len(rows)!=0:
                    self.product_table.delete(*self.product_table.get_children())
                    for row in rows:
